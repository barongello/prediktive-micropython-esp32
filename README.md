...
```
prediktive-sample-driver
  micropython.cmake
  prediktive_driver.c
  prediktive_driver.py
```

Let's start with the `prediktive_driver.c`
...
Then the `prediktive_driver.py` that will be frozen
...
The LED is on GPIO `32` and the button on the GPIO `33` (enabled with GND)
...
prediktive_driver.set_led(True)  # On
prediktive_driver.set_led(False) # Off
